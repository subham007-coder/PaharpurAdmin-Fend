  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (bannerData) {
        ...
        const response = await axios.put("http://localhost:5000/api/banner", newBanner);
        setBannerData(response.data);
      } else {
        // Add a new banner
        const response = await axios.post("http://localhost:5000/api/banner/create", newBanner);
        setBannerData(response.data);
      }
